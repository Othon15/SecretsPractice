...
const encrypt = require ("mongoose-encryption"); //aparaitito for using encryption after we installed our npm package
...
app.post("/register",function (req,res){
  const newUser = new User ({
    ...
    password:req.body.password   //// to bodyParser edw apeu8unetai sto name ="password" sto input type sto register.ejs
  });
...
app.post("/login",function(req, res){     //here we check in our database  if we have a user with the credetials they put in.
  ...
  const password = req.body.password;     // credentials    those are the two things that we have to check to find out if the user is in our database so he can login
